...
Z2 = X*Theta1';
A2=sigmoid(Z2);
A2= [ones(m, 1), A2]; % Add a column of ones to a2
A3=sigmoid(A2*Theta2');

h=A3';

%Put y to a maxtri only have 0 or 1 ,and the size of y is 10 by 5000
...
for j=1:m
    J=J+(1/m)*sum( -y(:,j).*log(h(:,j))-(1-y(:,j)).*log(1-h(:,j)) );  
end
...
a2 = a2';            %5000 X 25
a2 = [ones(m, 1),a2]; % Add a row of ones to a2, 5000 X 26 
a2 = a2';                 %26 X 5000
...
% step2:
Theta2 = Theta2(:,2:end);
delta2 = (Theta2') * delta3 .* sigmoidGradient(z2);   %25 X 5000
